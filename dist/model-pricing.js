...
export const PEAK_WINDOWS_BEIJING = [[9, 12], [14, 18]];
...
for (const entry of MODEL_PRICING_CATALOG) {
    for (const name of [entry.canonicalName, ...entry.aliases]) {
        ...
        if (key)
            MODEL_INDEX.set(key, entry);
    }
}
export function resolveModelPricing(...candidates) {
...
export function resolveCurrentModelPricing(model) {
    return resolveModelPricing(model?.id, model?.display_name);
}
export function currentModelLabel(model) {
...
export function pricingOrderOf(model) {
    ...
    if (!pricing)
        return MODEL_PRICING_CATALOG.length;
...
function costWith(prices, tokens) {
    return (tokens.input * prices.input
        + tokens.cacheRead * prices.cacheRead
        + tokens.cacheWrite * prices.cacheWrite
        + tokens.output * prices.output) / TOKENS_PER_MILLION;
}
export function calculateModelUsageCost(rawModel, tokens, utcTimestamp) {
    const model = resolveModelPricing(rawModel);
    if (!model)
        return null;
    const date = beijingDate(utcTimestamp);
    if (!date || (model.effectiveFrom && date < model.effectiveFrom))
        return null;
    let prices;
    let peak = null;
...
export function sessionOfFile(file, recordSessionId) {
...
export function isPeak(utcTimestamp) {
...
export function beijingDate(utcTimestamp) {
    const d = new Date(utcTimestamp);
    if (Number.isNaN(d.getTime()))
        return '';
